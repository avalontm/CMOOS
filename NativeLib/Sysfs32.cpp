// Sysfs32.cpp: implementation of the CSysfs32 class.
//
//////////////////////////////////////////////////////////////////////
...
)
{
	...
	{
		_pMirrorFile = new DiskImgFile32();
		_pMirrorFile->IniFSService(type);
	}
	else
		_pMirrorFile->CloseImgFile();

	_pMirrorFile->OpenImgFile(lpImageFile, uVDiskSize);
...
/*
HANDLE CSysfs32::FindFirstFileEx(
...
DWORD CSysfs32::GetCurrentDirectory(
...
BOOL CSysfs32::GetDiskFreeSpace(
...
DWORD CSysfs32::GetFullPathName(
...
BOOL CSysfs32::SetCurrentDirectory(
...
BOOL CSysfs32::CreateDirectoryEx(					// In order to increase speed of creating  directory in current directory
	LPCTSTR lpPathName,                         // pointer to directory path string
	LPSECURITY_ATTRIBUTES lpSecurityAttributes  // pointer to security descriptor
)
{
...
BOOL  CSysfs32::PathFileExist(LPCTSTR pszPath)
{
...
BOOL CSysfs32::InsertFile(IN const LPCSTR lpFileName,
	IN PBYTE  pBuffer,
...
BOOL CSysfs32::GetVolLabel(LPSTR lpLabel)
{
	return _pMirrorFile->GetVolLabel(lpLabel);
}

BOOL CSysfs32::CloseFile(HANDLE handle)
{
	return _pMirrorFile->CloseFileEx(handle);
}

void* CSysfs32::MirrorFile()
{
	return _pMirrorFile;
}

BOOL CSysfs32::IsFoulder(Flag fAttr)
{
	if (IsFile(fAttr))
		return FALSE;
...
BOOL CSysfs32::IsVolLabel(Flag fAttr)
{
	BOOL isVol = FALSE;
	if (IsLongDir(fAttr))
		return FALSE;
...
BOOL CSysfs32::RenameFile(LPCTSTR lpSrcFileName, LPCTSTR lpNewFileName)
{
	return _pMirrorFile->RenameFileEx(lpSrcFileName, lpNewFileName);
}

BOOL  CSysfs32::RefreshFatTable()
{
	return _pMirrorFile->RefreshFatTable();
}

// BOOL  CSysfs32::IsFileExist(LPCSTR lpFileName)
// {
