...
namespace CosmosNetwork.NET.ARP
{
    /// <summary>
    ...
    internal abstract class ARPPacket_Ethernet : ARPPacket
    {
        ...
        /// <exception cref="ArgumentException">Thrown if RawData is invalid or null.</exception>
        protected ARPPacket_Ethernet(ushort operation, MACAddress senderMAC, Address senderIP, MACAddress targetMAC, Address targetIP, int packet_size, MACAddress arpTargetMAC): base(targetMAC, senderMAC, 1, 0x0800, 6, 4, operation, packet_size)
        {
            for (int i = 0; i < 6; i++)
            {
