...
#define BYSPERSEC  512			// 每个扇区字节数
#define ROOTENTCNT	512			// 根目录表个数


typedef UINT Flag;
 
//xm.BootSector_BPB_RW
typedef struct _tagBootSector_BPB_RW  //引导扇区结构 --在读取引导扇区数据时用
{
	BYTE BS_jmpBoot[3];		//0
	CHAR BS_OEMName[8];		//3
...
//xm.BootSector_BPB
typedef struct _tagBootSector_BPB		//引导扇区结构  --编程时使用
{
	BYTE BS_jmpBoot[3];		//0
	CHAR BS_OEMName[8];		//3
