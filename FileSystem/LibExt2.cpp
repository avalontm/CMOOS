...
// 				exception e(strErr);throw e;
				char szInfo[256]={0};
				sprintf(szInfo, "DO NOT support file system type:%s",bType);
				printf(szInfo);
			}
			break;
		}
...
BOOL ReadBlock(CSysfs* pfs ,LPCTSTR psaFileName , DWORD dwBlock , BYTE*& pBuffer , DWORD& blocksize)
{
	...
	unsigned int ulLength = pfs->GetFileSize(hf,&ulFileLengthHigh);
	//printf("GetFileSize \n");
	unsigned int ulLengthReaded = 0;

	unsigned char* pbDecryptBuffer =new unsigned char[ulLength];
	pfs->ReadFile(hf,pbDecryptBuffer,ulLength,&ulLengthReaded,NULL);
	//printf("ReadFile \n");
	pfs->CloseFile(hf);
	//printf("CloseFile \n");
