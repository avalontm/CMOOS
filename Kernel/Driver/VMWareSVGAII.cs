...
namespace MOOS.Driver
{
    ...
    public class VMWareSVGAII 
    {
        ...
        public void Copy(uint x, uint y, uint newX, uint newY, uint width, uint height)
        {
            ...
            {
               Console.WriteLine("VMWareSVGAII Copy()");
            }
        }
        ...
        public VMWareSVGAII() 
        {
            this.device = PCI.GetDevice(0x15AD, 0x0405);
        ...
        */
        ...
        protected void TrimLine(ref int x1, ref int y1, ref int x2, ref int y2)
        {
            ...
            if (x1 == x2)
            {
                x1 = (int)Math.Min((byte)(width - 1), Math.Max(0, x1));
                x2 = x1;
                y1 = (int)Math.Min((byte)(height - 1), Math.Max(0, y1));
                y2 = (int)Math.Min((byte)(height - 1), Math.Max(0, y2));
