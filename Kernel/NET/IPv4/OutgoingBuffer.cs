                ...
                for (int e = 0; e < queue.Count; e++)
                {
                    if (queue[e].Status == BufferEntry.EntryStatus.ADDED)
                    {
                        if (IPConfig.IsLocalAddress(queue[e].Packet.DestinationIP) == false)
                        {
                            ...
                            {
                                ARPRequest_Ethernet arp_request = new ARPRequest_Ethernet(queue[e].NIC.MACAddress, queue[e].Packet.SourceIP, MACAddress.Broadcast, queue[e].nextHop, MACAddress.None);
                        ...
                        {
                            ARPRequest_Ethernet arp_request = new ARPRequest_Ethernet(queue[e].NIC.MACAddress, queue[e].Packet.SourceIP, MACAddress.Broadcast, queue[e].Packet.DestinationIP, MACAddress.None);
